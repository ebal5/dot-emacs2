...
(use-package migemo
  ...
  (let ((cand1 "/usr/local/share/migemo/utf-8/migemo-dict")
        (cand2 "/usr/share/cmigemo/utf-8/migemo-dict"))
    (if (file-exists-p cand1)
        (setq migemo-dictionary cand1)
      (if (file-exists-p cand2)
          (setq migemo-dictionary cand2)
        )
      )
    )
